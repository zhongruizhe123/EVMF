        ...
        for scorer, method in self.scorers:
            ...
            for vid_id in gt_vid_ids:
                ...
                if len(res[vid_id]) == 0 or len(gts[vid_id]) == 0:
                    ...
                    # print("gts[vid_id]", len(gts[vid_id]), len(res[vid_id]))
...
if __name__ == '__main__':
    # G:\image\MDVC-master-change\log\\0926235021\results_val_1_e0.json
    parser = argparse.ArgumentParser(description='Evaluate the results stored in a submissions file.')
    parser.add_argument('-s', '--submission', type=str,#35
                        default=r'D:\new_image\MDVC-master-change\log\1208010132\results_val_1_e37.json',
                        # G:\image\MDVC-master-change\log\MDVC_imgs\results _val_2_e0.json
                        help='sample submission file for ActivityNet Captions Challenge.')
    parser.add_argument('-r', '--references', type=str, nargs='+',
                        default=['D:\\new_image\\MDVC-master-change\\data\\val_1.json', 'D:\\new_image\\MDVC-master-change\\data\\val_2.json'],
                        help='reference files with ground truth captions to compare results against. delimited (,) str')
    parser.add_argument('--tious', type=float, nargs='+', default=[0.3, 0.5, 0.7, 0.9],  # [0.3, 0.5, 0.7, 0.9]
